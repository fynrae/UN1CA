...
GET_URL()
{
    ...
    eval "$CURL_CMD \"$KERNEL_URL\"" | jq -r --arg i "$1" '.assets[]? | select(.name | test($i)) | .browser_download_url' | head -n 1
}
# ]

KERNEL_ARCHIVE_URL="$(GET_URL "^UN1CA_Kernel-.*-a33x\.tar$")"
DTBO_ARCHIVE_URL="$(GET_URL "^UN1CA_DTBO-.*-a33x\.tar$")"
